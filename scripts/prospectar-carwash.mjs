/* Funil de prospecção: busca car washes por cidade (DuckDuckGo), visita o
   ...
   nada e em ritmo humano. Saída: prospeccao/dados.json + fotos/. */
import { writeFileSync, mkdirSync } from "node:fs";
import { firefox } from "playwright";

const CIDADES = [
  { q: "car wash orlando florida", cidade: "Orlando, FL" },
  { q: "car wash tampa florida", cidade: "Tampa, FL" },
  { q: "car wash phoenix arizona", cidade: "Phoenix, AZ" },
  { q: "car wash charlotte north carolina", cidade: "Charlotte, NC" },
];
...
for (const { q, cidade } of CIDADES) {
  try {
    ...
    for (const r of resultados) {
      ...
      candidatos.set(dominio, { nomeBusca: r.titulo, cidade, url: `https://${dominio}` });
      if (++porCidade >= 10) break;
    }
...
await browser.close();
writeFileSync("prospeccao/dados.json", JSON.stringify(sites, null, 2));
