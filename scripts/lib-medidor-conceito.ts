...
(function () {
  try {
    ...
    // próprio João não podem virar alerta nem entrar na conta. ?rvland=0
    // religa. ?rvland=teste manda tudo com a marca de teste: o Telegram
    // recebe com prefixo TESTE e nada é gravado.
    var teste = false;
    try {
      var modo = new URLSearchParams(location.search).get("rvland");
      if (modo === "1") localStorage.setItem(DESLIGA, "1");
      if (modo === "0") localStorage.removeItem(DESLIGA);
      teste = modo === "teste";
      if (!teste && localStorage.getItem(DESLIGA)) return;
    } catch (e) {}
    ...
    document.addEventListener("visibilitychange", function () {
      contar();
      // em modo teste a abertura vai uma vez só: a atualização de tempo viraria segundo aviso
      if (document.visibilityState === "hidden") { if (marcou && !teste) enviar(true); enviarCliques(true); }
    });
    window.addEventListener("pagehide", function () {
      contar();
      if (marcou && !teste) enviar(true);
      enviarCliques(true);
    });
  } catch (e) {}
})();
